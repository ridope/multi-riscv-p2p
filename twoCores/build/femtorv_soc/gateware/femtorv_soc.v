...
//
// Filename   : femtorv_soc.v
// Device     : 
// LiteX sha1 : b020d4cf
// Date       : 2022-05-14 10:42:32
//------------------------------------------------------------------------------
...
wire reset;
reg  [29:0] idbus_adr = 30'd0;
reg  [31:0] idbus_dat_w = 32'd0;
wire [31:0] idbus_dat_r;
reg  [3:0] idbus_sel = 4'd0;
reg  idbus_cyc = 1'd0;
reg  idbus_stb = 1'd0;
wire idbus_ack;
reg  idbus_we = 1'd0;
...
wire [3:0] mbus_wmask;
reg  [31:0] mbus_rdata0 = 32'd0;
wire mbus_rstrb;
reg  mbus_rbusy = 1'd0;
reg  mbus_wbusy = 1'd0;
reg  latch = 1'd0;
reg  [31:0] mbus_rdata1 = 32'd0;
...
reg  [29:0] idbus_adr_next_value0 = 30'd0;
reg  idbus_adr_next_value_ce0 = 1'd0;
reg  [31:0] idbus_dat_w_next_value1 = 32'd0;
reg  idbus_dat_w_next_value_ce1 = 1'd0;
reg  [3:0] idbus_sel_next_value2 = 4'd0;
reg  idbus_sel_next_value_ce2 = 1'd0;
reg  idbus_we_next_value3 = 1'd0;
reg  idbus_we_next_value_ce3 = 1'd0;
...
always @(*) begin
	mbus_rdata0 <= 32'd0;
	mbus_rdata0 <= mbus_rdata1;
	if (latch) begin
		mbus_rdata0 <= idbus_dat_r;
	end
end
always @(*) begin
	idbus_stb <= 1'd0;
	idbus_sel_next_value2 <= 4'd0;
	idbus_sel_next_value_ce2 <= 1'd0;
	idbus_we_next_value3 <= 1'd0;
	idbus_we_next_value_ce3 <= 1'd0;
	subfragments_next_state <= 1'd0;
	mbus_rbusy <= 1'd0;
	idbus_adr_next_value0 <= 30'd0;
	idbus_cyc <= 1'd0;
	idbus_adr_next_value_ce0 <= 1'd0;
	mbus_wbusy <= 1'd0;
	idbus_dat_w_next_value1 <= 32'd0;
	idbus_dat_w_next_value_ce1 <= 1'd0;
	latch <= 1'd0;
	...
	case (subfragments_state)
		...
		default: begin
			idbus_adr_next_value0 <= mbus_addr[31:2];
			idbus_adr_next_value_ce0 <= 1'd1;
			idbus_dat_w_next_value1 <= mbus_wdata;
			idbus_dat_w_next_value_ce1 <= 1'd1;
			idbus_sel_next_value2 <= mbus_wmask;
			idbus_sel_next_value_ce2 <= 1'd1;
			if ((mbus_rstrb | (mbus_wmask != 1'd0))) begin
				idbus_we_next_value3 <= (mbus_wmask != 1'd0);
				idbus_we_next_value_ce3 <= 1'd1;
...
always @(posedge sys_clk) begin
	...
	if (latch) begin
		mbus_rdata1 <= idbus_dat_r;
	end
	subfragments_state <= subfragments_next_state;
	if (idbus_adr_next_value_ce0) begin
		idbus_adr <= idbus_adr_next_value0;
	end
	if (idbus_dat_w_next_value_ce1) begin
		idbus_dat_w <= idbus_dat_w_next_value1;
	end
	if (idbus_sel_next_value_ce2) begin
		idbus_sel <= idbus_sel_next_value2;
	end
	if (idbus_we_next_value_ce3) begin
		idbus_we <= idbus_we_next_value3;
	end
...
reg [31:0] rom[0:5325];
initial begin
	$readmemh("femtorv_soc_rom.init", rom);
end
reg [31:0] rom_dat0;
always @(posedge sys_clk) begin
...
initial begin
	$readmemh("femtorv_soc_sram.init", sram);
end
reg [10:0] sram_adr0;
always @(posedge sys_clk) begin
	if (ram_we[0])
...
//------------------------------------------------------------------------------
// Memory mem: 55-words x 8-bit
//------------------------------------------------------------------------------
// Port 0 | Read: Sync  | Write: ---- | 
reg [7:0] mem[0:54];
initial begin
	$readmemh("femtorv_soc_mem.init", mem);
end
reg [5:0] mem_adr0;
always @(posedge sys_clk) begin
...
FemtoRV32 #(
	...
	.clk(sys_clk),
	.mem_rbusy(mbus_rbusy),
	.mem_rdata(mbus_rdata0),
	.mem_wbusy(mbus_wbusy),
	.reset((~(sys_rst | reset))),
	.mem_addr(mbus_addr),
	.mem_rstrb(mbus_rstrb),
	.mem_wdata(mbus_wdata),
	.mem_wmask(mbus_wmask)
);

endmodule

// -----------------------------------------------------------------------------
//  Auto-Generated by LiteX on 2022-05-14 10:42:32.
//------------------------------------------------------------------------------
